...
RUN curl -sSfL "https://release.anza.xyz/v3.1.13/install" -o /tmp/solana_install.sh && \
    ACTUAL=$(sha256sum /tmp/solana_install.sh | awk '{print $1}') && \
    test "$ACTUAL" = "7e52fd91dac613d7fc01cbd46123dd14f7790c5a394449dbe359ec0c6d305859" && \
