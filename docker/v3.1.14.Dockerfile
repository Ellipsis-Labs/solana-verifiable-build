...
RUN curl -sSfL "https://release.anza.xyz/v3.1.14/install" -o /tmp/solana_install.sh && \
    ACTUAL=$(sha256sum /tmp/solana_install.sh | awk '{print $1}') && \
    test "$ACTUAL" = "f2d4860409b14a7aacc6454431d3b5366f2795df696466fe5a7177b0adf5f761" && \
